...
//   if (appState === 'intro') {
//     return <QuantumIntro onComplete={handleIntroComplete} />;
//   }
...
//             className="bg-[#666666] text-white px-4 py-2 rounded border border-[#666666] hover:bg-[#555555] transition-colors"
//           >
//             DEMO LOGIN
//           </button>
//         </div>
...
//   const tabs = [
//     { id: 'chat' as TabType, label: 'Quantum Chat', icon: MessageSquare },
//     { id: 'monitor' as TabType, label: 'Security Monitor', icon: BarChart3 },
//     { id: 'settings' as TabType, label: 'Quantum Settings', icon: Settings },
//     { id: 'signup' as TabType, label: 'Sign Up/Login', icon: User },
//   ];
...
const handleInputSubmit = async (input: string) => {
  ...
  try {
    ...
    if (detection.isMalicious) {
      ...
      // Save to backend
      await axios.post(`http://localhost:${PORT}/api/chat/save`, {
  ...
  if (appState === 'auth') {
    return (
      <div>
        {!isLoggedIn ? <SignupLogin /> : <div className="text-center mt-20 text-[#666666]">You are already logged in.</div>}
        <div className="fixed bottom-4 right-4 z-50">
          <button
            onClick={handleLoginSuccess}
            className="bg-[#666666] text-white px-4 py-2 rounded border border-[#666666] hover:bg-[#555555] transition-colors"
